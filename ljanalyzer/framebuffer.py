...
class FrameBuffer:
    ...
    def __ensure_memory(self, max_size):
        ...
        """
        rtrn = min(self.__next_convenient_size(self.frame_count), max_size)
        required_memory = (
            rtrn * self.frame_dims[0] * self.frame_dims[1] * self.frame_dims[2]
        )
    ...
    def get_frame(self, index: int):
        ...
        if index < 0 or index >= self.size:
            raise IndexError(
                f"""index {index} out of bounds - must be between
                                0 and {self.size}"""
            )
